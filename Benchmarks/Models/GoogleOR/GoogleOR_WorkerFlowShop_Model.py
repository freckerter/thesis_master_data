class ORSolver(object):
    ...
    def solve_with_disunctive_model(self, config_path, init_path, sol_path, max_time):
        ...
        for station in range(products_initial[0][product], stations_count):
          for workstep in range(
            ...
            model.Add(task.end == task.start + min_duration +
                      cp_model.LinearExpr.WeightedSum(task.exists_vars, shifted))
      ...
      if status == cp_model.OPTIMAL or status == cp_model.FEASIBLE:
        ...
        solution_time = toc - tic
        solution_status = 'UNSOLVED'
      print("Test print config_path: " + config_path)
      print("Test print ini_path: " + init_path)
      print("Test print sol_path: " + sol_path)


      conf_f = config_path.split(".")[3].split("/")[4]
      ini_f = init_path.split(".")[3].split("/")[4]
      sol_f = sol_path.split(".")[1].split("/")[1]
...
configname = "S20-Ex16.xml"
initialname = "W20-Ex40.xml"
solutionname = configname.split(".")[0] + "_" + initialname.split(".")[0] + ".txt"


solver = ORSolver()
solver.solve_with_disunctive_model(config_path=f"./../Instances/XML_configurations/XML_configurations/{configname}",init_path=f"./../Instances/XML_initials/XML_initials/{initialname}", sol_path=f"./{solutionname}", max_time= 1000)
